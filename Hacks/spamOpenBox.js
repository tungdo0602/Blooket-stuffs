...
let boxName = bypass().window.prompt("Box name (space,...)")

function spamOpenBox(e){
    return findByProp("put").put("https://dashboard.blooket.com/api/users/unlockblook", {
    name: getStateNode().state.user.name,
    box: e.replace(e.at(0), e.slice(0,1).toUpperCase())}).then(res=>{
        if(!res.data.tokens<=20){
            console.log("Unlocked: " + res.unlockedBlook);
            window.setTimeout(()=>{
                spamOpenBox(e);
        })}else{
